            ...
            <section className="absolute w-[95%] h-[95%] bg-black rounded-[20px] z-10 text-center text-white">
              <header className="border border-white bg-[#ffffff29] rounded-xl p-3 mb-6 min-h-full min-w-full flex items-center">
                <section className="mt-4 space-y-4 px-2">
                  <h2 className="uppercase text-lg md:text-xl font-semibold text-transparent bg-clip-text bg-gradient-to-br from-yellow-500 to-red-500">{item.title}</h2>
                  <p className="md:text-lg text-white opacity-80">{item.description}</p>
                </section>
              </header>
            </section>
            <section className="absolute w-full h-[105%] bg-gradient-to-br from-yellow-500 to-red-500 animate-spin-low opacity-60" />
          </motion.article>
        ))}
      </section>
    </section>
